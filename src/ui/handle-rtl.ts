...
export const handleRtl = (
  ...
  let originalStyle = StyleSheet.flatten(style);

  // converting the style if the language is arabic
  if (style && lang === 'ar' && !noDirectionChange) {
    // 0-output style
    let outputStyle = { ...StyleSheet.flatten(style) };
    ...
    if (originalStyle.flexDirection) {
      ...
      if (
        !originalStyle.alignItems ||
        originalStyle.alignItems === 'flex-start'
      ) {
    ...
    if (originalStyle.right && originalStyle.left) {
      outputStyle.right = originalStyle.left;
      outputStyle.left = originalStyle.right;
